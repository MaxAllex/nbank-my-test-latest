...
public class UserSessionExtension implements BeforeEachCallback {
    ...
    public void beforeEach(ExtensionContext extensionContext) throws Exception {
        ...
        extensionContext.getTestMethod().ifPresent(method -> {
            ...
            CreateUserWithAccount createUserWithAccount = method.getAnnotation(CreateUserWithAccount.class);
    ...
    private void makeTransfer(MakeTransfer makeTransfer) {
        ...
        if (makeTransfer != null) {
            ...
            AccountResponse senderAccount = senderUser.getAccounts().get(makeTransfer.senderAccount() - 1);
            AccountResponse receiverAccount = receiverUser.getAccounts().get(makeTransfer.receiverAccount() - 1);
            SessionStorage.getAccountsSteps(senderUser).transferMoney(
                    senderAccount.getId(),
    ...
    private static TestUser getUserWithAccount(TestUser user, double amount, int howManyAccounts) {
        var accountsSteps = AccountsStepsFactory.byCredentials(user.getRequest().getUsername(), user.getRequest().getPassword());
