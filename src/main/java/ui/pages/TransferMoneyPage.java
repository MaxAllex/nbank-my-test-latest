...
public class TransferMoneyPage extends BasePage<TransferMoneyPage> {
  ...
  private final SelenideElement recipientAccountNumberField =
  ...
  private final SelenideElement searchButton =
      $("button[class='custom-btn shadow-custom blue-btn mt-3']");
  ...
  public TransferMoneyPage selectAccountSender(String account) {
    accountSelector.$("option[value='" + account + "']").shouldBe(Condition.exist);
    accountSelector.should(Condition.enabled).selectOptionByValue(account);
    return this;
  }

  public TransferMoneyPage enterRecipientName(String recipientName) {
    recipientNameField.shouldHave(Condition.enabled).setValue(recipientName);
    return this;
  }

  public TransferMoneyPage enterRecipientAccountNumber(String recipientAccountNumber) {
    recipientAccountNumberField.shouldHave(Condition.enabled).setValue(recipientAccountNumber);
    return this;
  }

  public TransferMoneyPage enterAmount(String amount) {
    amountField.shouldHave(Condition.enabled).setValue(amount);
    return this;
  }

  public TransferMoneyPage clickSendTransferButton() {
    sendTransferButton.shouldHave(Condition.enabled).click();
    return this;
  }

  public TransferMoneyPage clickConfirmDetailsCheckBox() {
    confirmDetailsCheckBox.shouldHave(Condition.enabled).click();
    return this;
  }

  public TransferMoneyPage clickTransferAgainButton() {
    transferAgainButton.shouldHave(Condition.enabled).click();
    return this;
  }
  ...
  public TransactionRow findFirstTransactionByType(String type) {
    return getAllTransactionRows().stream()
        ...
        .findFirst()
        .orElseThrow(() -> new AssertionError("Transaction with type [" + type + "] not found"));
  }

  public RepeatTransferModal repeatFirstTransactionByType(TransactionType type) {
    return findFirstTransactionByType(type.name()).clickRepeat();
  }
