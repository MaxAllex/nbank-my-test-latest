...
public class DTOGenerator {
  ...
  /**
   * Генерирует экземпляр указанного класса с случайными тестовыми данными. Работает на основе типов
   * полей и их имен (для более точной генерации). Для массивов генерируются пустые массивы, чтобы
   * избежать циклов.
   *
  ...
  public static <T> T generate(Class<T> clazz, Map<String, Object> overrides) {
    try {
      if (clazz.isRecord()) {
        ...
        if (constructors.length != 1) {
          throw new IllegalArgumentException(
              "Record должен иметь ровно один конструктор: " + clazz.getName());
        }
      ...
      if (constructors.length == 0) {
        throw new IllegalArgumentException(
            "Класс должен иметь публичный конструктор: " + clazz.getName());
      }
  ...
  private static Object generateValue(Class<?> type, String name) {
    if (type == long.class || type == Long.class) {
      ...
      if (name.toLowerCase().contains("username")) {
        String base =
            faker.name().firstName().replaceAll("[^A-Za-z0-9]", "").toLowerCase(Locale.ENGLISH);
        if (base.isBlank()) {
          base = "user";
        }
        int maxBaseLength = 6;
      ...
      } else if (name.toLowerCase().contains("accountnumber")
          || name.toLowerCase().contains("accounts")) {
  ...
  private static <T> void populateFields(T instance, Map<String, Object> overrides)
  ...
  private static Object[] buildArgsFromFields(
      Constructor<?> constructor, Class<?> clazz, Map<String, Object> overrides) {
