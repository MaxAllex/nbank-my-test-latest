...
public class HTTPRequestImp<Rec, Res> extends AbstractHTTPRequest implements HTTPRequest<Rec, Res> {
  ...
  public Res post(Rec model) {
    return StepLogger.log(
        "POST request to " + endpoint.getURL(),
        () ->
            given()
  ...
  public Res get() {
    return StepLogger.log(
        "GET request to " + endpoint.getURL(),
        () ->
            given()
                .spec(requestSpecification)
  ...
  public Res put(Rec model) {
    return StepLogger.log(
        "PUT request to " + endpoint.getURL(),
        () ->
            given()
                .spec(requestSpecification)
                .body(model)
  ...
  public Res patch(long id, Rec model) {
    return StepLogger.log(
        "PATCH request to " + endpoint.getURL(),
        () ->
            given()
                .spec(requestSpecification)
  ...
  public Res delete(long id) {
    return StepLogger.log(
        "DELETE request to " + endpoint.getURL(),
        () ->
            given()
                .spec(requestSpecification)
