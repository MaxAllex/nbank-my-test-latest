...
public class TransferMoneyUiTests extends BaseUiTest {
    ...
    public void transferMoneyTest() {
        ...
        new UserDashboardPage()
                ...
                .checkAlertMessageAndAccept(BankAlert.TRANSFER_MONEY_SUCCESSFULLY.getMessage(amount, receiver.getAccounts().getFirst().getAccountNumber()));

        AccountDao receiverAccountDaoFirst = DataBaseSteps.getAccountByAccountNumber(receiver.getAccounts().getFirst().getAccountNumber());
        AccountDao senderAccountDaoLast = DataBaseSteps.getAccountByAccountNumber(sender.getAccountNumber());
    ...
    public void transferMoneyWithNotFilledAmountTest() {
        ...
        new UserDashboardPage()
                ...
                .enterAmount(String.valueOf(amount))
                .clickSendTransferButton()
                .checkAlertMessageAndAccept(BankAlert.TRANSFER_AMOUNT_FILL_ALL_FIELDS_AND_CONFIRM.getMessage());
    }
    ...
    public void transferMoneyRepeatedTransferTest() {
        ...
        TransferMoneyPage page = new UserDashboardPage()
                .open()
                .clickToMakeTransferButton()
                .clickTransferAgainButton();

        List<TransactionRow> transactions = page.getAllTransactionRows().stream().toList();
        int expectedTransactionInCount = Math.toIntExact(transactions.stream().filter(x -> x.getTransactionType().equals(TransactionType.TRANSFER_IN.name())).count());
        int expectedTransactionOutCount = Math.toIntExact(transactions.stream().filter(x -> x.getTransactionType().equals(TransactionType.TRANSFER_OUT.name())).count());
        ...
        page.repeatFirstTransactionByType(TransactionType.TRANSFER_IN)
                ...
                .confirmDetails()
                .sendTransfer()
                .checkAlertMessageAndAccept(BankAlert.TRANSFER_AGAIN_SUCCESSFULLY.getMessage(amount, sender.getId(), receiver.getId()));

        List<TransactionRow> actualTransactions = new TransferMoneyPage()
                .open()
                .clickTransferAgainButton()
                .getAllTransactionRows();
        int actualTransactionInCount = Math.toIntExact(actualTransactions.stream().filter(x -> x.getTransactionType().equals(TransactionType.TRANSFER_IN.name())).count());
        int actualTransactionOutCount = Math.toIntExact(actualTransactions.stream().filter(x -> x.getTransactionType().equals(TransactionType.TRANSFER_OUT.name())).count());
    ...
    public void searchBySenderUsernameTest() {
        ...
        softly.assertThat(new UserDashboardPage()
                        .open()
                        .clickToMakeTransferButton()
                        .clickTransferAgainButton()
