...
public class TransferMoneyTests extends BaseTests {
  ...
  public void transferMoneyToOtherAccountTests(double transferAmount) {
    ...
    var userFirstAccount =
        userFirstAccounts.stream()
            .filter(account -> account.getId() == userFirst.getAccounts().getFirst().getId())
            .findFirst()
            .orElseThrow();
    var userLastAccount =
        userLastAccounts.stream()
    ...
    softly
        .assertThat(userFirstAccount.getBalance())
        .isCloseTo(userFirstExpectedAmount, offset(0.01));
    softly.assertThat(userLastAccount.getBalance()).isCloseTo(userLastExpectedAmount, offset(0.01));

    AccountDao accountDaoFirst =
        DataBaseSteps.getAccountByAccountNumber(userFirstAccounts.getFirst().getAccountNumber());
    AccountDao accountDaoLast =
        DataBaseSteps.getAccountByAccountNumber(userLastAccounts.getFirst().getAccountNumber());
  ...
  public void transferMoneyToMyAccountTests() {
    ...
    var userFirstAccountFirst =
        userFirstAccounts.stream()
            .filter(account -> account.getId() == userFirst.getAccounts().getFirst().getId())
            .findFirst()
            .orElseThrow();
    var userFirstAccountLast =
        userFirstAccounts.stream()
            .filter(account -> account.getId() == userFirst.getAccounts().getLast().getId())
            .findFirst()
            .orElseThrow();

    double userFirstExpectedAccountFirst =
        userFirst.getAccounts().getFirst().getBalance() - transferAmount;
    double userFirstExpectedAccountLast =
        userFirst.getAccounts().getFirst().getBalance() + transferAmount;
    ...
    AccountDao accountDaoFirst =
        DataBaseSteps.getAccountByAccountNumber(userFirstAccounts.getFirst().getAccountNumber());
    AccountDao accountDaoLast =
        DataBaseSteps.getAccountByAccountNumber(userFirstAccounts.getLast().getAccountNumber());
    DaoAndModelAssertions.assertThat(userFirstAccounts.getFirst(), accountDaoFirst).match();
  ...
  public void transferMoneyMoreThenPossibleTests(double amount) {
    ...
    ValidatableResponse transferredMoney =
        accountsSteps.transferMoney(
            userFirst.getAccounts().getFirst().getId(),
            userLast.getAccounts().getFirst().getId(),
            amount,
    ...
    softly
        .assertThat(
            users.getFirst().getAccounts().stream().allMatch(a -> a.getBalance() == expectedAmount))
        .isTrue();
    softly
        .assertThat(
            users.getLast().getAccounts().stream().allMatch(a -> a.getBalance() == expectedAmount))
        .isTrue();

    AccountDao accountDaoFirst =
        DataBaseSteps.getAccountById(users.getFirst().getAccounts().getFirst().getId());
  ...
  public void transferMoreMoneyThenIsInBalanceNegativeTests(double amount) {
    ...
    softly
        .assertThat(
            users.getFirst().getAccounts().stream().allMatch(a -> a.getBalance() == MAX_DEPOSIT))
        .isTrue();
    softly
        .assertThat(
            users.getLast().getAccounts().stream().allMatch(a -> a.getBalance() == MAX_DEPOSIT))
        .isTrue();
  ...
  public void transferMoneyByAdminNegativeTest() {
    ...
    getAdminAccountsSteps()
        .transferMoneyAuthByToken(
            userFirst.getAccounts().getFirst().getId(),
            userLast.getAccounts().getFirst().getId(),
            1,
