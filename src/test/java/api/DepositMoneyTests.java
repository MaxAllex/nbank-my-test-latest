...
public class DepositMoneyTests extends BaseTests {
  ...
  public void depositMoneyByUserTest(double amount) {
    ...
    AccountDao accountDao =
        DataBaseSteps.getAccountByAccountNumber(
            customerSteps.getCustomerAccounts().getFirst().getAccountNumber());
    DaoAndModelAssertions.assertThat(customerSteps.getCustomerAccounts().getFirst(), accountDao)
        .match();
  }
  ...
  public void depositMoneyFromSomeoneElseAccountNegativeTest() {
    ...
    getAccountsSteps(firstUser)
        .depositMoney(lastUser.getResponse().getId(), depositAmount, HttpStatus.SC_FORBIDDEN);
    List<CreateUserResponse> users =
        getAdminSteps().getUsers().stream()
            .filter(
                x ->
                    x.getId() == firstUser.getResponse().getId()
                        || x.getId() == lastUser.getResponse().getId())
            .toList();
    softly
        .assertThat(users)
        .allMatch(
            x -> Arrays.stream(x.getAccounts()).noneMatch(a -> depositAmount == a.getBalance()));
  }
  ...
  public void depositMoneyOnNonExistentAccountNegativeTest() {
    double depositAmount = 100;
    long nonExistentId =
        users.stream().mapToLong(x -> x.getResponse().getId()).max().getAsLong() + 100L;
    var firstUser = users.getFirst();
